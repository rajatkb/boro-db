...
func TestBitMapFreeList(t *testing.T) {
	...
	assert.True(t, freelist.TotalFreeLocs() == 37)

	freelist.ReleaseLoc([]uint64{10}) // no-op

	assert.Equal(t, uint8(255), bitmap[0])
	assert.Equal(t, uint8(3), bitmap[1])

	freelist.ReleaseLoc([]uint64{0, 1, 2})
	assert.Equal(t, uint8(0xF8), bitmap[0])

	pages, err = freelist.GetLocs(1)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(pages))
	assert.Equal(t, uint64(2), pages[0])

	pages, err = freelist.GetLocs(50)

	assert.Nil(t, err)
	assert.Equal(t, 39, len(pages))
	assert.True(t, freelist.TotalFreeLocs() == 0)
